        ...
        for y in  range(player_y-2,player_y+3):
            for x in range(player_x-2,player_x+3):
                if x <0 or y < 0:
                    ...
                    self.ids.map.add_widget(Label(text="🤖",font_size=30,font_name="D:/prog/RPG_/src/font/NotoEmoji-VariableFont_wght.ttf"))
                else:
                    self.ids.map.add_widget(Label(text=self.mapIco[str(self.manager.currentGame.Map.tiles[y][x])],font_size=30,font_name="D:/prog/RPG_/src/font/NotoEmoji-VariableFont_wght.ttf"))
